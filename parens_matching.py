...
Try looping through the string, keeping a count of how many open parentheses we
have.

Solution
We simply walk through the the string, starting at our input opening
parenthesis position. As we iterate, we keep a count of how many additional "("
we find as open_nested_parens. When we find a ")" we decrement
open_nested_parens. If we find a ")" and open_nested_parens is 0, we know that
...
The while loop keeps our space cost at O(1)O(1). It might be more Pythonic to
use:

  for char in sentence[position:]:

but then our space cost would be O(n), because in the worst case position would
be 0 and we'd take a slice of the entire input.
...
It's pretty cool when you can replace a whole data structure with a single
integer :)

...
Here we'd try 4^{12}
combinations (every combination of 4 colors for 12 nodes). In general, we'll have to check O(D^n) colorings. And that's not all—each time we try a coloring, we have to check all MM edges to see if the vertices at both ends have different colors. So, our runtime is O(M*D^n). That's exponential time since nn is in an exponent.
...
Each edge creates two neighbors: one for each node on either end. So when our code looks at every neighbor for every node, it looks at 2*M neighbors in all. With O(M)O(M) neighbors, collecting all the colors over the entire for loop takes O(M)O(M) time.
...
Well, each of our MM edges add two neighbors to the graph: one for each node on either end. So that's 2*M neighbors in total. Which means 2*M illegal colors in total.

But remember: we said we'd try as many as len(illegal_colors) + 1 colors per node. We still have to factor in that "+1"! Across all NN of our nodes, that's an additional NN colors. So we try 2*M+N colors in total across all of our nodes.
...
We care about polynomial time solutions (nn raised to a constant power, like O(n^2) because for large nns, polynomial time algorithms are more practical to actually use than higher runtimes like exponential time (a constant raised to the power of nn, like O(2^n). Computer scientists usually call algorithms with polynomial time solutions feasible, and problems with worse runtimes intractable.

...
Let's think about it. Each node has at most DD neighbors, and we have D+1 
...
Then, we look at all the colors (there are O(D)O(D) of them) to see which ones 
are available.
...
In a graph with maximum degree D, each node has at most DD neighbors. That 

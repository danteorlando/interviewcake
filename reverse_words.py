...
list of characters, do the in-place word reversal on that list, and re-join 
that list into a string before returning it. This isn't technically "in-place" 
and the list of characters will cost O(n)O(n) additional space, but it's a 
...
second character with the second to last character, and so on, moving towards 
the middle. Can you implement this in code?
...
We're using a cute one-liner to do the swap. In other languages you might need 
to do the swap by hand, recording one of the values in a temp variable.
...
Once we know the start and end indices of two words, how do we swap those two 
words?
...
Supposing we already knew the start and end indices of 'the' and 'landed', how 
long would it take to swap them?
...
So what'll be the total time cost with this approach? Assume we'll be able to 
learn the start and end indices of all of our words in just one pass 
(O(n) time).

O(n2) total time. Why? In the worst case we have almost as many words as we 
have characters, and we're always swapping words of different lengths. For 
example:
...
We have to move all nn characters except the first and last words, and a couple 
spaces. So we move n-5 characters in total.

For the third swap, we have another 55 characters we don't have to move. So we 
move n-10 in total. We'll end up with a series like this:

n + (n-5) + (n-10) + (n-15) + ... + 6 + 1 
This is a subsection of the common triangular series.  We're just skipping 4 
terms between each term!

So we have the sum of "every fifth number" from that triangular series. That 
means our sum will be about a fifth the sum of the original series! But in big 
O notation dividing by 5 is a constant, so we can throw it out. The original 
triangular series is O(n^2), and so is our series with every fifth element!

Okay, so O(n^2) time. That's pretty bad. It's possible that's the best we can 
do...but maybe we can do better?
...
The character reversal reverses the words! It's a great first step. From there, 
we just have to "unscramble" each word.

More precisely, we just have to re-reverse each word!

Solution
We'll write a helper function reverse_characters() that reverses all the 
characters in a string between a front_index and back_index. We use it to:

Reverse all the characters in the entire message, giving us the correct word 
order but with each word backwards.
...
Complexity
O(n) time and O(n) space. Our space cost comes from converting the message 
string to a list. If our input was a list, our space cost would be O(1) because 
we'd be using a constant amount of additional space beyond the input.

Hmm, the team used your function to finish deciphering the message. There 
definitely seems to be a plot brewing, but no specifics on where. The sender 
seemed to really like Eccles cakes. Any ideas?
...
string first. This put all the words in the correct spots, but with the 
characters in each word backwards. So to get the final answer, we reversed the 
characters in each word. This all takes two passes through the string, so O(n) 
time total.

This might seem like a blind insight, but we derived it by using a clear 
strategy:

Solve a simpler version of the problem (in this case, reversing the characters 
instead of the words), and see if that gets us closer to a solution for the 
original problem.

We talk about this strategy in the "get unstuck" section of our coding 
interview tips.

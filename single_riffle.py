...
To prove this, let's write a function to tell us if a full deck of cards shuffled_deck is a 
...
Why do I care? A single riffle is not a completely random shuffle. If I'm right, I can make more informed bets and get rich 
...
Breaking the problem into smaller subproblems will clearly involve reducing the size of at least one of our stacks of cards. 
...
If shuffled_deck is a riffle of half1 and half2, then the first card from shuffled_deck should be either the same as the 
...
Looking at three face up stacks of cards, the top card of the shuffled deck is 32, the top card of half 1 is 7, 
and the top card of half 2 is 10.

Now that we know the first card checks out, how do we get to our subproblem?


Let's "throw out" the top card from shuffled_deck as well as the card it matched with from the top of half1 or half2. 
...
So we keep doing this over and over until we exhaust shuffled_deck. 
If we get to the end and each card "checks out," we return True.

How do we implement this in code?

Now that we have a problem that's the same as the original problem except smaller, our first thought might be to use recursion. 
All we need is a base case. What's our base case?

We stop when we run out of cards in our shuffled_deck. So that's our base case: when we've checked all cards in shuffled_deck, 
we return True because we know all of the cards have been "accounted for."
...
Before we talk about optimization, note that our inputs are of small and constant size. 
...
But if we're going to do something inefficient, we should at least know about it. Great engineers have both the skill to see how 
to optimize their code and the wisdom to know when those optimizations aren't worth it. At this point in the interview I recommend 
saying "I think we can optimize this a bit further, although given the constraints on the input this probably won't be very 
...
If shuffled_deck has n items to start, taking our first slice takes n-1 time and space (plus half that, since we're also slicing one of our 
halves, but that's just a constant multiplier so we can ignore it). In our second recursive call, slicing takes n-2 time and space. Etcetera.
...
One way we could to that is to avoid slicing and instead keep track of indices in the list:
...
What's happening in each iteration of our recursive function? Sometimes we're taking a card off of half1 
and sometimes we're taking a card off of half2, but we're always taking a card off of shuffled_deck.
...
If the current card in shuffled_deck is the same as the top card from half1, increment half1_index and keep going. 
This can be thought of as "throwing out" the top cards of half1 and shuffled_deck, reducing the problem to the 
remaining cards in the stacks.
...
What We Learned
If you read the whole breakdown section, you might have noticed that we started with a recursive function that looked simple but 
turned out to cost O(n^2) time and space because each recursive step created its own new "slice" of the input list. 
If you missed that part, go back and take a look.

Be careful of the hidden time and space costs of list slicing! Consider tracking list indices "by hand" instead 
